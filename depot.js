...
function depotSeries(positions, charts, bench, rateOf) {
  ...
  const usable = positions.filter(p => charts.get(p.sym) && p.shares > 0 && Number(p.avg) > 0);
  ...
  for (const p of usable) {
    ...
    if (r == null || now == null) continue;
    const c = p.shares * Number(p.avg) * r;
    cost += c;
  ...
  await Promise.all(items.map(p => new Promise(resolve => {
    Queue.push(async () => {
      ...
      rows.push({
        ...
        priceConv: (price != null && rate != null) ? price * rate : null,
        avgConv: (avg > 0 && rate != null) ? avg * rate : null,
        ok: !!chart,
      });
      resolve();
    });
...
function dpRow(r) {
  ...
  return `<tr data-sym="${esc(r.sym)}">
    ...
    <td class="r">${dpCell(r.avgConv, r.avg, r.cur)}</td>
...
function dpOpenForm(existing) {
  ...
  const preview = () => {
    ...
    /* Kurs je Anteil in Anzeigewährung – direkt unter dem Eingabefeld,
       auch schon ohne eingetragene Stückzahl. */
    const eurBox = $("#dppriceeur");
    if (eurBox) {
      if (av > 0 && rate != null && fxBase(cur) !== target) {
        eurBox.textContent = "≈ " + fmtNum(av * rate, target, 2) + " je Anteil";
    ...
    const cost = sh * av;
    const inTarget = (rate != null && fxBase(cur) !== target)
      ? ` <span class="dp-conv">= ${fmtNum(cost * rate, null, 2)} ${target}</span>` : "";
    let html = `<b>${fmtNum(sh, null, sh % 1 ? 3 : 0)}</b> Anteile × <b>${fmtNum(av, cur, 2)}</b>
                = Einstand <b>${fmtNum(cost, cur, 2)}</b>${inTarget}`;
    let warn = false;
    if (market != null) {
