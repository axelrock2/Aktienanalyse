...
async function loadChart(symbol, range = "2y") {
  ...
  const out = { meta: r.meta || {}, t: [], c: [], h: [], l: [], v: [] };
...
function localSearch(q, limit = 9) {
  ...
  for (const x of scored) {
    ...
    out.push({ s: r[0], n: r[1], c: r[2], e: r[3], sec: r[4] });
...
async function remoteSearch(q) {
  ...
  const out = data.quotes
    ...
    .map(x => ({ s: x.symbol, n: x.longname || x.shortname || x.symbol, c: "", e: x.exchDisp || x.exchange || "", sec: x.quoteType === "ETF" ? "ETF" : "", remote: true }));
  Cache.set(key, out);
  return out;
}
...
function suggRow(r) {
  ...
  row.innerHTML = `<div class="sg-main">
      ...
      <div class="sg-meta"><span class="sg-sym">${esc(r.s)}</span>
        ${r.e ? `<span class="tag">${esc(r.e)}</span>` : ""}${r.c ? `<span class="tag">${esc(r.c)}</span>` : ""}${r.sec ? `<span class="tag">${esc(r.sec)}</span>` : ""}</div></div>
...
/* Chart */
function drawChart(chart, a, days) {
  ...
  chartObj = new Chart(cv, {
    ...
    options: {
      ...
      plugins: { legend: { labels: { color: "#93A0B0", boxWidth: 18, font: { family: "Inter", size: 11 } } },
        tooltip: { backgroundColor: "#202833", borderColor: "#313B49", borderWidth: 1, titleColor: "#E9EDF3", bodyColor: "#93A0B0" } },
      scales: {
        x: { ticks: { color: "#6B7787", maxTicksLimit: 8, font: { family: "IBM Plex Mono", size: 10 } }, grid: { display: false } },
        y: { ticks: { color: "#6B7787", font: { family: "IBM Plex Mono", size: 10 } }, grid: { color: "rgba(49,59,73,.5)" } },
      },
    },
  });
}
...
function buildDossier(item,chart,fund,a,deep,news,bench,demo){
  const cur=chart.meta.currency||"";
  ...
  <div class="ds-wrap">
    ...
    <!-- 7 Bilanzqualität -->
    <section class="ds-sec"><h2>7 · Bilanzqualität ${dsBadge("partial")}</h2>
      <div class="ds-grid2">${dsRow("Verschuldung (D/E)",fund&&fund.debtToEquity!=null?fN(fund.debtToEquity,0):"n. v.")}</div>
      <p class="dsna">Piotroski F-Score, Altman Z-Score und Beneish M-Score erfordern vollständige Mehrjahres-Bilanzen (GuV, Bilanz, Cashflow über 5+ Jahre). Diese Daten liefert die freie Quelle nicht – Felder bewusst offen statt mit Schätzwerten gefüllt.</p>
    </section>
    ...
    <section class="ds-sec"><h2>8 · Bewertungsanalyse ${dsBadge("calc")} ${dsBadge("fetch")}</h2>
      ...
      <p class="dshint">DCF und Reverse-DCF verlangen Cashflow-Projektionen und einen WACC – bewusst nicht automatisch geschätzt ${dsBadge("judge")}. Multiplikatoren dienen dem Peer- und Historienvergleich.</p>
    </section>
...
async function openDossier(item){
  ...
  try{
    ...
    panel.innerHTML = `<div class="p-in">${buildDossier(item, chart, fund, a, deep, news, bench, demo)}</div>`;
