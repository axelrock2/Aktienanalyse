...
FEEDS = [
    ...
    # --- Englisch / international (reine Wirtschafts-/Finanzquellen) ---
    ("Reuters Business", "en", "https://www.reutersagency.com/feed/?best-topics=business-finance&post_type=best"),
    ...
    ("MarketWatch Markets", "en", "https://feeds.content.dowjones.io/public/rss/mw_marketpulse"),
...
# Eindeutig themenfremde Schlagzeilen aussortieren, falls ein allgemeinerer
# Feed doch mal Sport/Promi/Unterhaltung hereinmischt. Konservativ gehalten,
# damit keine echten Wirtschaftsmeldungen faelschlich entfernt werden.
OFFTOPIC = [
    "super bowl", "nfl", "nba", "bundesliga", "champions league", "olympia",
    "olympics", "fussball", "fußball", "tennis", "formel 1", "formula 1",
    "hollywood", "kino", "movie", "film premiere", "grammy", "oscar", "promi",
    "celebrity", "royal", "kardashian", "taylor swift", "rezept", "horoskop",
    "wetter heute", "spielbericht", "transfermarkt",
]
...
def is_offtopic(title: str) -> bool:
    low = " " + title.lower() + " "
    for w in OFFTOPIC:
        # Wortgrenzen, damit z. B. "nfl" nicht in "Inflation" trifft
...
def parse_feed(raw: bytes, source: str, lang: str) -> list[dict]:
    ...
    for node in entries[:MAX_PER_SOURCE * 2]:
        ...
        if not title or not link or not link.startswith("http"):
            continue
        if is_offtopic(title):        # eindeutig themenfremde Meldungen ueberspringen
            continue
