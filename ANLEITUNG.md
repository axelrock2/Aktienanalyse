...
| Qualitäts-Score zeigt „n. v." | Der Titel wird von der Kennzahlen-Quelle nicht geführt – das betrifft vor allem ETFs (dort ist der Score bewusst leer) und kleine Auslandsnotierungen ohne US-Handel. Steht eine US-Notierung zur Verfügung, ist sie der zuverlässigere Eintrag. Timing-Score und Zonen funktionieren unabhängig davon. |
| Kennzahlen erscheinen erst nach ein paar Sekunden | Normal bei Titeln außerhalb der Watchlist: Sie werden beim Öffnen live geholt (etwa zwei Sekunden) und dann zwölf Stunden zwischengespeichert. Wer einen Titel häufig ansieht, trägt ihn in `scripts/fundamentals_watchlist.txt` ein – dann ist er sofort da. |
| Depot zeigt keinen Index-Vergleich | Beabsichtigt, solange bei den Positionen kein „gehalten seit" steht: Ohne Kaufdatum gibt es keinen gemeinsamen Zeitraum, und ein Vergleich zweier verschieden langer Zeiträume wäre irreführend. Datum eintragen, dann erscheint der Vergleich. |
